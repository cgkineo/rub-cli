...
class Commands {
  ...
  static finished() {
    ...
    if (!Commands._wasHandled) {
      Commands._unhandledCommandCallbacks.forEach((callback)=>{
        callback();
      });
      return;
    }
    ...
    if (Commands._wasHandled) {
      Commands._finishedLoadingCallbacks.forEach((callback)=>{
        callback();
      });
      return;
    }
  ...
  static has(expected) {
    ...
    for (let i = 0, l = expected.length; i < l; i++) {
      ...
      let isUndefined = (expect === undefined);
      let isAtEnd = (expected.length-1 <= i);
  ...
  static get(type, name) {
    ...
    switch (type) {
      ...
      case "command":
        return _.find(commands._cmdObjects, (cmd)=>{
